...
class DQN:
    def __init__(
        ...
        self.device = device
        self.eval_net, self.target_net = Net(num_states, num_actions, device), Net(
...
def run(num_people, max_ep_len, memory_capacity, args, seed):
    ...
    for i in range(episodes):
        ...
        if i % 10 == 0:
            print(i, "-------------------------------")
            print("done", ep_reward, env.loans)
...
def main():
    ...
    for i in range(num_exps):
        ...
        save_plot_avg(reward_list, args, i + 1, num_exps, num_people, max_ep_len)


def save_plot_avg(reward_list_all, args, curr, num_exps, num_people, max_ep_len):
    ...
    ax.set_ylabel("Sum of Fairness Reward")
