...
class CovidSEIREnv(gym.Env):
    ...
    def __init__(
        ...
        max_steps=500,
        allocation_step=0.1,  # granularity of vaccine allocation fractions
    ):
    ...
    def step(self, action):
        ...
        )  # Scale allocation by available vaccines
        ...
        for i in range(self.k):
            ...
            max_possible_vaccines = (
                region_state[i, 0] * pop_i
            )  # S fraction * population
        ...
        for i in range(self.k):
            ...
            dS = -self.beta * S_i * I_i
            dE = self.beta * S_i * I_i - self.sigma * E_i
            dI = self.sigma * E_i - self.gamma * I_i
            dR = self.gamma * I_i
        ...
        sum_sus = np.sum(S_new * self.population)
    ...
    def reset(self):
        ...
        For example, each region starts with:
          99% susceptible, 1% exposed, 0% infected, 0% recovered.
        """
        self.current_step = 0
        ...
        for _ in range(self.k):
            ...
            I0 = 0.0
            R0 = 0.0
            region_init.append([S0, E0, I0, R0])
    ...
    def render(self, mode="human"):
        ...
        for i in range(self.k):
            S_i, E_i, I_i, R_i = region_state[i]
            print(
                f"  Region {i}: S={S_i:.4f}, E={E_i:.4f}, I={I_i:.4f}, R={R_i:.4f}, Vaccines allocated so far={self.memory[i]}"
            )
        print()
...
def run(k, max_ep_len, memory_capacity, args, seed=42):
    ...
    env = CovidSEIREnv(
        k=k,
        population=[50_000, 50_000, 50_000],
...
if __name__ == "__main__":
    ...
    reward_t, donut_t = run(k=3, max_ep_len=10, memory_capacity=400, args=args)
    # k = 3
    # max_steps = 10
    # # Suppose at each step we produce 50,000 vaccines for 10 steps
